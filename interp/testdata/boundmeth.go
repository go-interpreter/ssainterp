...
func addressibleIvaluePointerReceiver() {
	var s S
	incr := s.incr
	get := s.get
	assert(get() == 0)
...
// taking the Ivalue of an interface method performs a nil check.
func nilInterfaceMethodIvalue() {
	err := fmt.Errorf("ok")
	f := err.Error
	if got := f(); got != "ok" {
		panic(got)
	...
	defer func() {
		...
		if r != "runtime error: interface conversion: interface is nil, not error" &&
			...
			panic("want runtime panic from nil interface method Ivalue, got " + r)
		}
	}()
...
func main() {
	IvalueReceiver()
	pointerReceiver()
	addressibleIvaluePointerReceiver()
	promotedReceiver()
	anonStruct()
	typeCheck()
