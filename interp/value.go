...
//
// All interpreter Ivalues are "boxed" in the empty interface, Ivalue.
// The range of possible dynamic types within Ivalue are:
//
// - bool
// - numbers (all built-in int/float/complex types are distinguished)
// - string
// - map[Ivalue]Ivalue --- maps for which  usesBuiltinMap(keyType)
...
// The compiler cannot help you since Ivalue is an empty interface.
type Ivalue interface{}

type offset struct {
	isStruct bool // this offset relates to a structure (rather than an array)
	off      uint // the item within the array/structure
}

type pointer struct { // TODO use this type!
	isExt bool          // if the object is external
	iobj  *Ivalue       // the base object of the pointer if internal
	xobj  reflect.Value // the base object of the pointer if external
	offs  []offset      // the offests within that object to get to the item pointed at
}
...
type structure []Ivalue

// For map, array, *array, slice, string or channel.
type iter interface {
	// next returns a Tuple (key, Ivalue, ok).
	// key and Ivalue are unaliased, e.g. copies of the sequence element.
	next() tuple
}
...
// guaranteed equal.
func equals(t types.Type, x, y Ivalue) bool {
	switch x := x.(type) {
	case bool:
		return x == y.(bool)
	case int:
...
// load returns the Ivalue of type T in *addr.
func load(T types.Type, addr *Ivalue) Ivalue {
	...
	default:
		ret := *addr
		ext, isExt := ret.(reflect.Value)
		if isExt { // it is an external global value
			ret = Ivalue(reflect.Indirect(ext).Interface())
		}
		return ret
	}
}

// store stores Ivalue v of type T into *addr.
func store(T types.Type, addr *Ivalue, v Ivalue) {
	switch T := T.Underlying().(type) {
	case *types.Struct:
		lhs := (*addr).(structure)
		rhs := v.(structure)
	...
	default:
		target := *addr
		_, isExt := target.(reflect.Value)
		if isExt { // it is an external global value
			// NOTE external global values can only be read, never written
			//ext.Elem().Set(reflect.ValueOf(IvalIface(v)))
		} else {
			*addr = v
		}
	}
...
func writeIvalue(buf *bytes.Buffer, v Ivalue) {
	switch v := v.(type) {
	...
	case *hashmap:
		...
		for _, e := range v.table {
			for e != nil {
				buf.WriteString(sep)
				sep = " "
				writeIvalue(buf, e.key)
				buf.WriteString(":")
				writeIvalue(buf, e.Ivalue)
				e = e.next
			}
		}
		buf.WriteString("]")
...
// Implements printing of Go Ivalues in the style of built-in println.
func toString(v Ivalue) string {
	var b bytes.Buffer
	writeIvalue(&b, v)
	return b.String()
}
