...
func makeReflectIvalue(t types.Type, v Ivalue) Ivalue {
	return structure{rtype{t}, v}
}

// Given a reflect.Value, returns its rtype.
func rV2T(v Ivalue) rtype {
	return v.(structure)[0].(rtype)
}

// Given a reflect.Value, returns the underlying interpreter Ivalue.
func rV2V(v Ivalue) Ivalue {
	return v.(structure)[1]
}

// makeReflectType boxes up an rtype in a reflect.Type interface.
func makeReflectType(rt rtype) Ivalue {
	return iface{rtypeType, rt}
}

func ext۰reflect۰Init(fr *frame, args []Ivalue) Ivalue {
	// Signature: func()
	return nil
}

func ext۰reflect۰rtype۰Bits(fr *frame, args []Ivalue) Ivalue {
...
func ext۰reflect۰rtype۰Elem(fr *frame, args []Ivalue) Ivalue {
...
func ext۰reflect۰rtype۰Field(fr *frame, args []Ivalue) Ivalue {
	...
	return structure{
		...
		[]Ivalue{}, // TODO(adonovan): indices
		f.Anonymous(),
	}
}

func ext۰reflect۰rtype۰In(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype, i int) int
	i := args[1].(int)
	return makeReflectType(rtype{args[0].(rtype).t.(*types.Signature).Params().At(i).Type()})
}

func ext۰reflect۰rtype۰Kind(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) uint
	return uint(reflectKind(args[0].(rtype).t))
}

func ext۰reflect۰rtype۰NumField(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) int
	return args[0].(rtype).t.Underlying().(*types.Struct).NumFields()
}

func ext۰reflect۰rtype۰NumIn(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) int
	return args[0].(rtype).t.(*types.Signature).Params().Len()
}

func ext۰reflect۰rtype۰NumMethod(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) int
	return fr.i.prog.MethodSets.MethodSet(args[0].(rtype).t).Len()
}

func ext۰reflect۰rtype۰NumOut(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) int
	return args[0].(rtype).t.(*types.Signature).Results().Len()
}

func ext۰reflect۰rtype۰Out(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype, i int) int
	i := args[1].(int)
	return makeReflectType(rtype{args[0].(rtype).t.(*types.Signature).Results().At(i).Type()})
}

func ext۰reflect۰rtype۰Size(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) uintptr
	return uintptr(fr.i.sizes.Sizeof(args[0].(rtype).t))
}

func ext۰reflect۰rtype۰String(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) string
	return args[0].(rtype).t.String()
}

func ext۰reflect۰New(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.Type) reflect.Value
	t := args[0].(iface).v.(rtype).t
	alloc := fr.i.zero(t)
	return makeReflectIvalue(types.NewPointer(t), &alloc)
}

func ext۰reflect۰SliceOf(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) Type
	return makeReflectType(rtype{types.NewSlice(args[0].(iface).v.(rtype).t)})
}

func ext۰reflect۰TypeOf(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.rtype) Type
	return makeReflectType(rtype{args[0].(iface).t})
}

func ext۰reflect۰ValueOf(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (interface{}) reflect.Value
	itf := args[0].(iface)
	return makeReflectIvalue(itf.t, itf.v)
}

func ext۰reflect۰Zero(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (t reflect.Type) reflect.Value
	t := args[0].(iface).v.(rtype).t
	return makeReflectIvalue(t, fr.i.zero(t))
}
...
func ext۰reflect۰Value۰Kind(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) uint
	return uint(reflectKind(rV2T(args[0]).t))
}

func ext۰reflect۰Value۰String(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) string
	return toString(rV2V(args[0]))
}

func ext۰reflect۰Value۰Type(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) reflect.Type
	return makeReflectType(rV2T(args[0]))
}

func ext۰reflect۰Value۰Uint(fr *frame, args []Ivalue) Ivalue {
...
func ext۰reflect۰Value۰Len(fr *frame, args []Ivalue) Ivalue {
	...
	case map[Ivalue]Ivalue:
		return len(v)
	default:
		panic(fmt.Sprintf("reflect.(Ivalue).Len(%v)", v))
	}
}

func ext۰reflect۰Value۰MapIndex(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) Ivalue
	tIvalue := rV2T(args[0]).t.Underlying().(*types.Map).Key()
	k := rV2V(args[1])
	switch m := rV2V(args[0]).(type) {
	case map[Ivalue]Ivalue:
		if v, ok := m[k]; ok {
			return makeReflectIvalue(tIvalue, v)
		}
	...
	return makeReflectIvalue(nil, nil)
}

func ext۰reflect۰Value۰MapKeys(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) []Ivalue
	var keys []Ivalue
	...
	case map[Ivalue]Ivalue:
		for k := range v {
			keys = append(keys, makeReflectIvalue(tKey, k))
		}

	case *hashmap:
		for _, e := range v.table {
			for ; e != nil; e = e.next {
				keys = append(keys, makeReflectIvalue(tKey, e.key))
			}
		}
...
func ext۰reflect۰Value۰NumField(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) int
	return len(rV2V(args[0]).(structure))
}

func ext۰reflect۰Value۰NumMethod(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) int
	return fr.i.prog.MethodSets.MethodSet(rV2T(args[0]).t).Len()
}

func ext۰reflect۰Value۰Pointer(fr *frame, args []Ivalue) Ivalue {
...
func ext۰reflect۰Value۰Index(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (v reflect.Value, i int) Ivalue
	i := args[1].(int)
	t := rV2T(args[0]).t.Underlying()
	switch v := rV2V(args[0]).(type) {
	case array:
		return makeReflectIvalue(t.(*types.Array).Elem(), v[i])
	case []Ivalue:
		return makeReflectIvalue(t.(*types.Slice).Elem(), v[i])
	default:
		panic(fmt.Sprintf("reflect.(Ivalue).Index(%T)", v))
	}
}

func ext۰reflect۰Value۰Bool(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) bool
	return rV2V(args[0]).(bool)
}

func ext۰reflect۰Value۰CanAddr(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (v reflect.Value) bool
	// Always false for our representation.
	return false
}

func ext۰reflect۰Value۰CanInterface(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (v reflect.Value) bool
	// Always true for our representation.
	return true
}

func ext۰reflect۰Value۰Elem(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (v reflect.Value) reflect.Value
	switch x := rV2V(args[0]).(type) {
	case iface:
		return makeReflectIvalue(x.t, x.v)
	case *Ivalue:
		return makeReflectIvalue(rV2T(args[0]).t.Underlying().(*types.Pointer).Elem(), *x)
	default:
		panic(fmt.Sprintf("reflect.(Ivalue).Elem(%T)", x))
	}
}

func ext۰reflect۰Value۰Field(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (v reflect.Value, i int) reflect.Value
	v := args[0]
	i := args[1].(int)
	return makeReflectIvalue(rV2T(v).t.Underlying().(*types.Struct).Field(i).Type(), rV2V(v).(structure)[i])
}

func ext۰reflect۰Value۰Float(fr *frame, args []Ivalue) Ivalue {
...
func ext۰reflect۰Value۰Interface(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (v reflect.Value) interface{}
	return ext۰reflect۰valueInterface(fr, args)
}

func ext۰reflect۰Value۰Int(fr *frame, args []Ivalue) Ivalue {
	...
	case int64:
		return x
	default:
		panic(fmt.Sprintf("reflect.(Ivalue).Int(%T)", x))
	}
}

func ext۰reflect۰Value۰IsNil(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) bool
	switch x := rV2V(args[0]).(type) {
	case *Ivalue:
		return x == nil
	...
	case *closure:
		return x == nil
	default:
		panic(fmt.Sprintf("reflect.(Ivalue).IsNil(%T)", x))
	}
}

func ext۰reflect۰Value۰IsValid(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (reflect.Value) bool
	return rV2V(args[0]) != nil
}

func ext۰reflect۰Value۰Set(fr *frame, args []Ivalue) Ivalue {
	// TODO(adonovan): implement.
	return nil
}

func ext۰reflect۰valueInterface(fr *frame, args []Ivalue) Ivalue {
	// Signature: func (v reflect.Value, safe bool) interface{}
	v := args[0].(structure)
	return iface{rV2T(v).t, rV2V(v)}
}

func ext۰reflect۰error۰Error(fr *frame, args []Ivalue) Ivalue {
	return args[0]
}
...
func initReflect(i *interpreter) {
	...
	// variables, making zero Ivalues, loading, and storing.
	//
