...
// constIvalue returns the Ivalue of the constant with the
// dynamic type tag appropriate for c.Type(), cacheing as appropriate.
func (i *interpreter) constIvalue(c *ssa.Const) Ivalue {
	/*
	   	// NOTE this caching approach is not always faster,
	   	// so only use it where more than 1 referer to the constant.
	   	// In the current SSA implementation, it is possible there never are duplicates.

	   	refs := c.Referrers()
	   	cache := refs != nil && len(*refs) > 1

	   	if cache {
	   		i.constantsRWMutex.RLock()
	   		val, found := i.constants[c]
	   		i.constantsRWMutex.RUnlock()
	   		if found {
	   			//fmt.Printf("DEBUG const cache hit %v:%T\n", val, val)
	   			return val
	   		}
	   	}

	   	//panic(fmt.Sprintf("DEBUG const cache miss %v:%T\n", val, val))
	   	ret := constIvalueEval(c, i)
	   ...
	   // constIvalueEval returns the Ivalue of the constant with the
	   // dynamic type tag appropriate for c.Type().
	   func constIvalueEval(c *ssa.Const, i *interpreter) Ivalue {
	*/
	if c.IsNil() {
		return i.zero(c.Type()) // typed nil
	}
...
// zero returns a new "zero" Ivalue of the specified type.
func (i *interpreter) zero(t types.Type) Ivalue {

	/* TODO early tests show small benefit from caching zero values, retest
		i.zeroesRWMutex.RLock()
		zv := i.zeroes.At(t)
		i.zeroesRWMutex.RUnlock()
		_, isIvalue := zv.(Ivalue)
		if isIvalue {
			//fmt.Printf("DEBUG zeroes cache hit %s = %v:%T\n", t, zv, zv)
			return zv
		}
		zv = zeroEval(t, i)
		i.zeroesRWMutex.Lock()
		i.zeroes.Set(t, zv)
		i.zeroesRWMutex.Unlock()
		//fmt.Printf("DEBUG zeroes cache miss %s = %v:%T\n", t, zv, zv)
		return zv

	}

	// zeroEval returns a new "zero" Ivalue of the specified type.
	func zeroEval(t types.Type, i *interpreter) Ivalue {
	*/
	switch t := t.(type) {
	case *types.Basic:
		if t.Kind() == types.UntypedNil {
			panic("untyped nil has no zero Ivalue")
		}
	...
	case *types.Array:
		...
		return a
	case *types.Named:
		return i.zero(t.Underlying())
	case *types.Interface:
		return iface{} // nil type, methodset and Ivalue
	case *types.Slice:
		return []Ivalue(nil)
	case *types.Struct:
		s := make(structure, t.NumFields())
		for ii := range s {
			s[ii] = i.zero(t.Field(ii).Type())
		}
		return s
	case *types.Tuple:
		if t.Len() == 1 {
			return i.zero(t.At(0).Type())
		}
		s := make(tuple, t.Len())
		for ii := range s {
			s[ii] = i.zero(t.At(ii).Type())
		}
		return s
	case *types.Chan:
		return chan Ivalue(nil)
	case *types.Map:
		if usesBuiltinMap(t.Key()) {
			return map[Ivalue]Ivalue(nil)
		}
		return (*hashmap)(nil)
...
func slice(x, lo, hi, max Ivalue) Ivalue {
	var Len, Cap int
	switch x := x.(type) {
	case string:
		Len = len(x)
...
func (i *interpreter) lookup(instr *ssa.Lookup, x, idx Ivalue) Ivalue {
	...
	case map[Ivalue]Ivalue, *hashmap:
		...
		if !ok {
			v = i.zero(instr.X.Type().Underlying().(*types.Map).Elem())
		}
		if instr.CommaOk {
			v = tuple{v, ok}
		}
...
// dynamic type.
//
func binop(op token.Token, t types.Type, x, y Ivalue) Ivalue {
	switch op {
	case token.ADD:
		switch x.(type) {
		case int:
...
// If t is a reference type, at most one of x or y may be a nil Ivalue
// of that type.
//
func eqnil(t types.Type, x, y Ivalue) bool {
	...
	case *types.Map, *types.Signature, *types.Slice:
		...
		case map[Ivalue]Ivalue:
			return (x != nil) == (y.(map[Ivalue]Ivalue) != nil)
		case *ssa.Function:
			switch y := y.(type) {
			case *ssa.Function:
				return (x != nil) == (y != nil)
...
func (i *interpreter) unop(instr *ssa.UnOp, x Ivalue) Ivalue {
	switch instr.Op {
	case token.ARROW: // receive
		v, ok := <-x.(chan Ivalue)
		if !ok {
			v = i.zero(instr.X.Type().Underlying().(*types.Chan).Elem())
		}
		if instr.CommaOk {
			v = tuple{v, ok}
		}
	...
	case token.MUL:
		return load(deref(instr.X.Type()), x.(*Ivalue))
	case token.NOT:
		return !x.(bool)
	case token.XOR:
		switch x := x.(type) {
...
// It returns the extracted Ivalue on success, and panics on failure,
// unless instr.CommaOk, in which case it always returns a "Ivalue,ok" tuple.
//
func typeAssert(i *interpreter, instr *ssa.TypeAssert, itf iface) Ivalue {
	var v Ivalue
	err := ""
	if itf.t == nil {
	...
	if err != "" {
		...
		return tuple{i.zero(instr.AssertedType), false}
	}
	if instr.CommaOk {
		return tuple{v, true}
	}
...
func write(fr *frame, fd int, b []byte) (int, error) {
	...
	if fr != nil && fr.i != nil &&
		fr.i.CapturedOutput != nil && (fd == 1 || fd == 2) {
		fr.i.capturedOutputMu.Lock()
		defer fr.i.capturedOutputMu.Unlock()
		return fr.i.CapturedOutput.Write(b)
	}
...
func callBuiltin(caller *frame, callpos token.Pos, fn *ssa.Builtin, args []Ivalue) Ivalue {
	switch fn.Name() {
	case "append":
		if len(args) == 1 {
			return args[0]
		...
		// append([]T, ...[]T) []T
		return append(args[0].([]Ivalue), args[1].([]Ivalue)...)

	case "copy": // copy([]T, []T) int or copy([]byte, string) int
		src := args[1]
		if _, ok := src.(string); ok {
			params := fn.Type().(*types.Signature).Params()
			src = caller.i.conv(params.At(0).Type(), params.At(1).Type(), src)
		}
		return copy(args[0].([]Ivalue), src.([]Ivalue))

	case "close": // close(chan T)
		close(args[0].(chan Ivalue))
		return nil

	case "delete": // delete(map[K]Ivalue, K)
...
func rangeIter(x Ivalue, t types.Type) iter {
	switch x := x.(type) {
	case map[Ivalue]Ivalue:
		...
		// reflect.(Ivalue).MapKeys machinery.
		it := make(mapIter)
		go func() {
			for k, v := range x {
				it <- [2]Ivalue{k, v}
...
// widen widens a basic typed Ivalue x to the widest type of its
// category, one of:
...
// conv converts the Ivalue x of type t_src to type t_dst and returns
// the result.
// Possible cases are described with the ssa.Convert operator.
//
func (i *interpreter) conv(t_dst, t_src types.Type, x Ivalue) Ivalue {
	...
	//    + string -> []byte/[]rune.
	//
	// All are treated the same: first we extract the Ivalue to the
	...
	case *types.Basic:
		...
		if ut_src.Kind() == types.UnsafePointer {
			...
			//
			// return (*Ivalue)(x.(unsafe.Pointer))
			// creates a new pointer of a different
			// type but the underlying interface Ivalue
